...
namespace KSPSerialIO
{
	...
	public class SettingsNStuff : MonoBehaviourExtended
    {
    ...
    public class KSPSerialPort : MonoBehaviourExtended
    {
        ...
        private void Begin()
        {
			Debug.Log ("KSPSerialIO: creating instance of SerialPort");
			...
			Debug.Log ("KSPSerialIO: Connecting to " + PortNumber + " at " + SettingsNStuff.BaudRate);
            Port.ReceivedBytesThreshold = 3;
			Port.DataReceived += Port_ReceivedEvent;

			if (SettingsNStuff.NoDetect) {
				Debug.Log ("KSPSerialIO: Setting port to: " + SettingsNStuff.DefaultPort);
				PortNumber = SettingsNStuff.DefaultPort;
			}
			...
			if (!Port.IsOpen) {
				...
				Port.Open ();
				Debug.Log ("KSPSerialIO: port open");

			} else {
				Debug.Log ("KSPSerialIO: Port is already open");
			}
        ...
        */
				...
				try {
                        ...
                        foreach (string PortName in names)
                        {
							...
							if (!Port.IsOpen)
                            {
                                ...
                                {
                                    Port.Open();
									Debug.Log("KSPSerialIO: port is open");
                                }
                                catch (Exception e)
                                {
                                ...
                                if (Port.IsOpen)
                                {
									Debug.Log("KSPSerialIO: handshake");
								...
								Debug.Log("KSPSerialIO: port is not open");
							}
                            }
                            else
                            {
		...
		public void Port_ReceivedEvent(object sender, SerialDataReceivedEventArgs e)
        {
			Debug.Log ("KSPSerialIO: packet received");
        ...
        {
            byte calc_CS;
			Debug.Log ("KSPSerialIO: Processing COM");
            if (rx_len == 0)
            {
    ...
    [KSPAddon(KSPAddon.Startup.Flight, false)]
	public class KSPSerialIO : MonoBehaviourExtended
    {
...
//				KSPSerialPort.updatePort ();
            ...
            else
            {
				...
				Debug.Log ("Vessel: " + FlightGlobals.ActiveVessel.ToString() + " port: " + KSPSerialPort.Port.IsOpen.ToString());
                //ActiveVessel.OnFlyByWire -= new FlightInputCallback(AxisInput);
            }
